...
async function fileSend(ctx, path, root) {
    await send(ctx, path, { root: root || ctx.session.dir || __dirname + '/view' })
        .then(v => {
            console.log('url:', v);
        })
        .catch(e => {
...
router.post('/history', koaBody, async(ctx) => {
    ctx.response.body = {
        status: ctx.session.history ? true : false,
...
router.get('/', async(ctx) => {
    ctx.session = {
        dir: false,
        history: ctx.session.history && Object.keys(ctx.session.history).length > 0 ? ctx.session.history : defaultHistory
    }
