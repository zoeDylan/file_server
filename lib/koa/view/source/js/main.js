        ...
        getDir: function(dir) {
            ...
            _data.getDir(this.dirPath, (res) => {
                if (res.status) {
                    ...
                    $('#__dirList').css('margin-top', $('.inputContent').height() + 10);
                } else {
                    _dir.error = true;
                    _dir.msg = res.error;
                }
...
$('input').keyup((e) => {
    switch (e.keyCode) {
        ...
        case 38:
            _downItemNum--;
            _downItemNum = _downItemNum < 0 ? $('.history .list-group button').length - 1 : _downItemNum;
            $('input').val($('.history .list-group button').eq(_downItemNum).html());
            $('.history .list-group button').removeClass('list-group-item-success').eq(_downItemNum).addClass('list-group-item-success')
            break;
        case 40:
            _downItemNum = _downItemNum > $('.history .list-group button').length - 1 ? 0 : _downItemNum;
            $('input').val($('.history .list-group button').eq(_downItemNum).html());
            $('.history .list-group button').removeClass('list-group-item-success').eq(_downItemNum).addClass('list-group-item-success')
            _downItemNum++;
            break;
...
$('input').on('focus', () => {
    _downItemNum = 0;
    $('.history .list-group button').removeClass('list-group-item-success');
